...
#FILEPATH = "C:/Users/mjh250/Documents/Local mjh250/mjh250/2018_02_16/BackgroundSubtracted_CheckingBgndFirst.hdf5"
#FOLDERPATH = "scan3"
#datafile = datafile.DataFile(FILEPATH,mode="r")
TAG = "Raman_White_Light_0Order_Processed_Image.*"

TAGS = ["Infinity3_First_Processed_Image",
        "Infinity3_Second_Processed_Image",
        "Raman_Laser_0Order_Processed_Image",
        "Raman_Laser_Spectrum_Processed_Image",
        "Raman_White_Light_0Order_Processed_Image",
        "Raman_White_Light_Spectrum_Processed_Image"]


#TAGS = ["Infinity3_Bias_Image",
...
#"Raman_Bias_0Order_int",
#"Raman_Bias_Spectrum_int",
#"Raman_Bias_Spectrum_wl",
#"Raman_Laser_0Order_atBkgndLoc_int",
#"Raman_Laser_0Order_int",
#"Raman_Laser_Spectrum_atBkgndLoc_int",
#"Raman_Laser_Spectrum_atBkgndLoc_wl",
#"Raman_Laser_Spectrum_int",
#"Raman_Laser_Spectrum_wl",
#"Raman_White_Light_0Order_int",
#"Raman_White_Light_Bkgnd_0Order_int", 
#"Raman_White_Light_Bkgnd_Spectrum_int",
#"Raman_White_Light_Bkgnd_Spectrum_wl",
#"Raman_White_Light_Spectrum_int",
#"Raman_White_Light_Spectrum_wl"]
#TAG = TAGS[10]
#TAG = "Raman_White_Light_0Order_int.*"
print "TAG",TAG
...
class ShapeAnalysis():
    def get_ellipse_contour(self,input_image):
    ...
    def make_vertex_mask(self,vertices,image_shape):
    ...
    def apply_snake(self,image,contour):
    ...
    def run_watershed(self,image,markers):
    ...
    def kmeans(self,image,nsegments):
    ...
    def plot_multiscale(self,images,figname,valid = None):
    ...
    def get_largest_binary_convex_hull(self,image):
        #assume image is binary
        return skimage.morphology.convex_hull_object(image)
    
    def get_lowest_level_set(self,image):
        level_set = np.zeros(image.shape)
        level_set[image < 0.25] = 1
        return level_set

    
    def process_bbox_centre(self, image):
        contour = self.get_ellipse_contour(image)
        if contour == []:
            return [[0,0],[0,0],[0,0],[0,0]], [int(np.round(image.shape[1]/2)), int(np.round(image.shape[0]/2))]
        #print("contour: "+str(contour))
        bbox,boxlims = self.get_bounding_box(contour)
        
        return bbox, [int(np.round(np.mean([boxlims[0], boxlims[2]]))), int(np.round(np.mean([boxlims[1], boxlims[3]])))]
    
    def process_image(self,image,figname):
        contour = self.get_ellipse_contour(image)
        print("contour: "+str(contour))
        #ellipse = cv2.fitEllipse(contour)
    
        bbox,boxlims = self.get_bounding_box(contour)
        mask = self.make_vertex_mask(bbox,image.shape)
        ...
        level_set = self.get_lowest_level_set(prod)
        #particle_contour = self.get_ellipse_contour(level_set)
        #particleMask = make_vertex_mask(particle_contour,prod.shape)
        np_only = prod-prod*((self.get_largest_binary_convex_hull(level_set)-1)*-1)
        ...
        def has_valid_segment(image,max_radius = 5):
            separated = self.separate_segments(image)
            valid_segments = []
    
            for i in range(len(separated)):
                img = separated[i]
                (cx,cy) = self.CoM_image(img)
        ...
        self.plot_multiscale(images,figname,valid="Filled: {0},Edge: {1}".format(len(filled_valid_segments) > 0, len(edge_valid_segments) > 0))
    
        return images
    
    def zero_pad(self,img,xdim,ydim):
    ...
    def separate_segments(self,image):
    ...
    def main(self):
        ...
        # limit = len(images)
        limit = 0 # for testing, do only a few images
        multiscale_images = []
            
        for i,image in enumerate(images):
            if i>=0 and i <= limit:
                print(i)
                output_img =self.process_image(image,"fig{}".format(i))
        ...
        for imgs in multiscale_images:
            ...
            print("shape: "+str(imgs[9].shape))
            print("COM:"+str(self.CoM_image(imgs[9])))
            plt.imshow(imgs[9])
