...
class MainDialog(QMainWindow, window.Ui_MainWindow):
        ...
        for fileNum in range(len(filepaths)):
            ...
            for sname in data:
                if sname.startswith('ParticleScannerScan_'):
                    ...
                    subparticle_number_sublist = []
                    for pname in data[sname]:
                        print('pname: ' + str(pname))
                        if pname.startswith('Particle_'):
                            particle_num_sublist.append(len(particle_num_sublist))
                            subparticle_num_subsublist = []
                            for subpname in data[sname+'/'+pname]:
                                if subpname.startswith('SubParticle_'):
                                    subparticle_num_subsublist.append(len(subparticle_num_subsublist))
                            subparticle_number_sublist.append(subparticle_num_subsublist)
                    particle_number_list.append(particle_num_sublist)
                    subparticle_number_list.append(subparticle_number_sublist)
            ...
            # Handle special cases with incomplete data
            if '2018_04_28' in filepath:
                del scan_number_list[-1]
                del particle_number_list[-1]
                del subparticle_number_list[-1]
    ...
    #                            "timestamp", datetime.datetime.now().isoformat())
                        ...
                        pimg.attrs.create("creation timestamp", datafile['Raman_Laser_0Order_int'].attrs['creation_timestamp'])
                        ...
                        # --- GET OOPTICS DATA ---
                        data_line = np.array(
                                datafile['OceanOptics_DF_Spectrum_int'])
                        pimg = pdata.create_dataset(
                                "OceanOptics_DF_Spectrum",
                                data=data_line)
                        pimg.attrs.create(
                                "timestamp", datetime.datetime.now().isoformat())
                        pimg.attrs.create(
                          "wavelengths", datafile['OceanOptics_DF_Spectrum_wl'])
                        
                        # =============================================================================
                        # --- Do Time Scans ---
                        # if particle_number == 12:
                        #     no_of_timeslots = 7
                    if time_scan:
                        for subparticle_number in subparticle_number_list[scan_number][particle_number]:
                            print('Time slot: ' + str(subparticle_number))
                            
                            # --- Print name of file being analyzed to track progress
                            print('Processing ParticleScannerScan_%s/Particle_%s/SubParticle_%s'
                                  % (scan_number, particle_number, subparticle_number))
        
                            # --- GET SCAN DATA & INITIALIZE PROCESSED FILE ---
                            datafile = scan_analyzer.getScanDataSetGeneral(
                                                    data, scan_number, particle_number, subparticle_number)
                            processedData = h5py.File(processed_filepath, "a")
                            pdata = processedData.create_group(
                                'scan%s/particle%s/subparticle%s' % (scan_number, particle_number, subparticle_number))
        ...
        #                            "timestamp", datetime.datetime.now().isoformat())
                            ...
                            if "2017_12_14" in filepath:
                                ...
                                if ((bgndSum > badBgndThreshold) and not (oldbgndSum > badBgndThreshold)):
                                    ...
                                    for pn in range(1,len(particle_number_list[scan_number])-particle_number):
                                        print("Looking forward by " + str(pn) + " particles for a good background.")
                                        tmp_datafile = scan_analyzer.getScanDataSetGeneral(
                                                        data, scan_number, particle_number+pn, subparticle_number)
                                        tmp_background_image = np.array(tmp_datafile['Raman_Laser_0Order_atBkgndLoc_int'])
                                        tmpBgndCrop = (tmp_background_image[yBotCrop:(data_image.shape[0]-yTopCrop),
                                                 xLeftCrop:(data_image.shape[1]-xRightCrop)])
                                        if (tmpBgndCrop.sum() < badBgndThreshold):
                            ...
                            # --- RAMAN LASER SPECTRUM ---
                            bgnd_datafile = scan_analyzer.getScanDataSetGeneral(
                                                    data, scan_number, current_bgnd_particle_number, subparticle_number)
                            data_image = np.array(datafile['Raman_Laser_Spectrum_int'])
                            bias_image = np.array(datafile['Raman_Bias_Spectrum_int'])
                            background_image = np.array(
                                    bgnd_datafile['Raman_Laser_Spectrum_atBkgndLoc_int'])
    ...
    def btnRunSWA_clicked(self):
        ...
        for fileNum in range(len(filepaths)):
            ...
            try:
                ...
                shutil.os.mkdir(processed_filepath+'/LineGraphs')
            except OSError:
                shutil.os.mkdir(processed_filepath)
                shutil.os.mkdir(processed_filepath+'/spots')
                shutil.os.mkdir(processed_filepath+'/rings')
                ...
                if sname.startswith('scan'):
                    ...
                    for pname in data[sname]:
                        if pname.startswith('particle'):
                            ...
                            subparticle_num_subsublist = []
                            for subpname in data[sname+'/'+pname]:
                                if subpname.startswith('subparticle'):
                                    subparticle_num_subsublist.append(int(subpname[11:]))
                            subparticle_number_sublist.append(subparticle_num_subsublist)
                    particle_number_list.append(particle_num_sublist)
                    subparticle_number_list.append(subparticle_number_sublist)
            ...
            # Find range of counts to which images shall be scaled, and timestamps:
            pixel_max_list_Raman0Order = []
            pixel_max_list_White0Order = []
            list_timestamps = []
            print("Finding colourbar scaling parameters and timestamps...")
            for n, scan_number in enumerate(scan_number_list):
                pixel_max_sublist_Raman0Order = []
                pixel_max_sublist_White0Order = []
                sublist_timestamps = []
                for m, particle_number in enumerate(particle_number_list[n]):
            ...
            pixel_max_list_Raman0Order_r = copy.deepcopy(pixel_max_list_Raman0Order)
            pixel_max_list_White0Order_r = copy.deepcopy(pixel_max_list_White0Order)
            
            Raman0OrderMax = []
            White0OrderMax = []
            for n, scan_number in enumerate(scan_number_list):
                for i in range(0, min(10, len(particle_number_list[n]))):
            ...
            DF_denoising_weight = 5  # Amount of denoising to apply to DF spectra (0 is no smoothing)
            OOptics_denoising_weight = 1  # Amount of denoising to apply to Ocean Optics spectra (0 is no smoothing)
            subtract_raman_PL = False  # Whether or not to fit and subtract background from the Raman Line spectrum
    ...
    #                    plt.close()
                        ...
                        fig.savefig(processed_filepath + '/' + identities[particle_number] +
                            '/scan' + str(scan_number) +
                            'particle' + str(particle_number) +
                            '_RamanSpectrumLine.png',
                            bbox_inches='tight')
                        plt.close()
    
                        # --- DARK FIELD SPECTRUM IMAGE AND LINE PLOT ---
                        ...
                        ax.plot(data_wavelengths, data_line)
                        plt.minorticks_on()
                        ax.set_title("Dark field spectrum (Andor)")
                        ...
                        # --- OCEAN OPTICS PLOT ---
                        dset = datafile['OceanOptics_DF_Spectrum']
                        data_line = np.array(dset)
                        data_wavelengths = dset.attrs['wavelengths']
                        data_wavelengths = np.asarray([x for x in data_wavelengths if x >= 500])
                        data_line = np.asarray(data_line[data_line.size-data_wavelengths.size:])
                        data_wavelengths = np.asarray([x for x in data_wavelengths if x <= 800])
                        data_line = np.asarray(data_line[:data_wavelengths.size])
                        data_line, value, status = smoothing.convex_smooth(data_line, weight=OOptics_denoising_weight, objective_type="quadratic", normalise=True)
                        fig = plt.figure()
                        ax = plt.subplot(111)
                        ax.plot(data_wavelengths, data_line)
                        plt.minorticks_on()
                        ax.set_title("Dark field spectrum")
                        ax.set_xlabel(r"$\lambda$ (nm)")
                        ax.set_ylabel("Intensity")
                        fig.savefig(processed_filepath + '/' + identities[particle_number] +
                                    '/scan' + str(scan_number) +
                                    'particle' + str(particle_number) +
                                    '_OOpticsSpectrum.png',
                                    bbox_inches='tight')
                        plt.close()
                    ...
                    # --- Do Time Scans ---
                    if time_scan:
                        for subparticle_number in subparticle_number_list[n][particle_number]:
                            # --- Decide how to normalize images
                            if per_image_normalization:
                                white_normalization_target = pixel_max_list_White0Order[n][particle_number][subparticle_number]
                                normalization_target = pixel_max_list_Raman0Order[n][particle_number][subparticle_number]
                            ...
                            # --- Print name of file being analyzed to track progress
                            print('Processing scan%s/particle%s/subparticle%s' % (scan_number,
                                                                    particle_number, subparticle_number))
                            ...
                            fig.savefig(processed_filepath + '/' + identities[particle_number] +
                                        '/scan' + str(scan_number) +
                                        'particle' + str(particle_number) + 'subparticle' + str(subparticle_number) +
                                        '_Inf3.png',
                                        bbox_inches='tight')
                            plt.close()
        ...
        #                    plt.close()
                            ...
                            # --- PLOT DF AND RAMAN SIDE-BY-SIDE
                            contrast_enhancement_factor = 0.9 # >0, bigger is darker
                            pixel_min = 0 # lower limit of the colorscale
                            f, (ax1, ax2) = plt.subplots(1, 2, sharex=False, sharey=False)
                            ...
                            f.suptitle('Time slot ' + str(subparticle_number), y = 0.8)
                            ...
                            # --- RAMAN LASER SPECTRUM IMAGE AND LINE ---
                            dset = datafile['Raman_Laser_Spectrum_Processed_Image']
                            data_image = np.array(dset)
                            data_image = data_image[:,spectrum_left_crop:]
                            data_image = remove_cosmic_rays(data_image, cosmic_ray_threshold)
                            ...
                            data_line = np.asarray(data_line[data_line.size-data_wavelengths.size:])
                            data_line, value, status = smoothing.convex_smooth(data_line, weight=raman_denoising_weight, objective_type="quadratic", normalise=True)
                            data_line_bgdfit = baseline_als(data_line, 10^12, 0.00002, niter=10)
                            fig = plt.figure()
                            ax = plt.subplot(111)
                            ...
                            # --- DARK FIELD SPECTRUM IMAGE AND LINE ---
                            dset = datafile['Raman_White_Light_Spectrum_Processed_Image']
                            data_image = np.array(dset)
                            data_image = data_image[:,spectrum_left_crop:]
                            data_image = remove_cosmic_rays(data_image, cosmic_ray_threshold)
...
def remove_cosmic_rays(input_image, threshold):
    median_filter_size = 5
    median_image = ndimage.median_filter(input_image, size=median_filter_size)
    mad = ndimage.median_filter(np.abs(input_image - median_image), size=median_filter_size)
    mads_from_median = np.divide(np.abs(input_image - median_image), mad)
    mask = mads_from_median > threshold
    mask = np.array(mask, dtype=np.uint8)
    no_of_cosmic_rays = np.sum(mask)
    print("No of cosmic rays = " + str(no_of_cosmic_rays))
    print("MAD: " + str(mad))
    print("Max mads_from_median: " + str(np.max(mads_from_median)))
    ret, mask = cv2.threshold(mask,0,255,cv2.THRESH_BINARY)
    dilated_mask = cv2.dilate(mask, np.ones(3), iterations = 2)
    dilated_mask = dilated_mask > 0
    #  Smooth out bright pixels
    output_image = np.where(dilated_mask, median_image, input_image)
    return output_image
...
class ParticleScanAnalysis:
    ...
    def getScanDataSetGeneral(self, datafile, scan_number, particle_number, subparticle_number = None):
        if subparticle_number is None:
            try:
            ...
            dataset = datafile['/ParticleScannerScan_%s/Particle_%s/SubParticle_%s'
                                       % (scan_number, particle_number, subparticle_number)]
        return dataset
    ...
    def getProcessedScanDataSet(self, datafile, scan_number, particle_number, subparticle_number = None):
        if subparticle_number is None:
            dataset = datafile['/scan%s/particle%s'
                               % (scan_number, particle_number)]
        else:
            dataset = datafile['/scan%s/particle%s/subparticle%s'
                               % (scan_number, particle_number, subparticle_number)]
        return dataset
