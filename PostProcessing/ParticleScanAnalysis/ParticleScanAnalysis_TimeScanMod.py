...
class MainDialog(QMainWindow, window.Ui_MainWindow):
    ...
    def btnRun_clicked(self):
        ...
        for fileNum in range(len(filepaths)):
            ...
            print('Processing ParticleScannerScan_0/Particle_0')

            # --- GET SCAN DATA & INITIALIZE PROCESSED FILE ---
            datafile = data['/ParticleScannerScan_0/Particle_0']
            processedData = h5py.File(processed_filepath, "a")
            pdata = processedData.create_group('scan0/particle0')
...
#                            "timestamp", datetime.datetime.now().isoformat())
            ...
            imgMax = zimg.max()
            imgThumbIntegral = (
                    zimg[50:(data_image.shape[0]-50),
                         750:(data_image.shape[1]-750)].sum())
...
# --- Iterate through particle scans and process files
            # --- Print name of file being analyzed to track progress
            print('Processing ParticleScannerScan_0_0/Particle_1')

            # --- GET SCAN DATA & INITIALIZE PROCESSED FILE ---
            datafile = data['/ParticleScannerScan_0_0/Particle_1']
            processedData = h5py.File(processed_filepath, "a")
            pdata = processedData.create_group('scan0_0/particle1')
