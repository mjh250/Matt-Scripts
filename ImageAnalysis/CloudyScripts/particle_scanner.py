...
"""
"""
import camera, prior_stage, camera_stage_mapper, seabreeze, spectrometer_aligner, Uniblitz_Shutter
import traits
...
#        self._abort_scan_event.clear()
    def tile_scans(self, size, background = True, tile_start_function=None, ts_args=[], ts_kwargs={}, *args, **kwargs):
        def worker_function():
            ...
            number_of_particles = len([k for k in latest_group.keys() if 'z_scan_' in k])            
    ...
    def take_zstacks_of_particles(self, dz=np.arange(-2.5,2.5,0.2), datafile_group = None, *args, **kwargs):
        ...
        self.go_to_particles(self.pf_align_and_take_z_scan(dz,g), *args, **kwargs)
    ...
    def take_zstacks_of_particles_tiled(self, shape, **kwargs):
        ...
        self.tile_scans(shape, 
                        ...
                        payload_function=self.pf_align_and_take_z_scan(datafile_group=g,**kwargs))
    ...
    def pf_align_and_take_z_scan(self, dz=np.arange(-4,4,0.4), datafile_group=None):
        ...
        def align_and_take_z_scan():
            ...
            image = self.csm.camera.color_image()
            img = g.create_dataset("camera_image",data=image[image.shape[0]/2-50:image.shape[0]/2+50, image.shape[1]/2-50:image.shape[1]/2+50])
            ...
            here = stage.position()            
            laser_focus = raman.AlignHeNe(dset)
            ...
            time.sleep(0.3)
            raman.take_fast_kinetic()
            
            "save the data to the HDF5 output file"
            Raman_spc = g.create_dataset("kinetic_raman",data=raman.kinetic_scan_data)
...
#===================================================================
            ...
            #self.aligner.optimise_2D(tolerance=0.03,stepsize=0.1)
            self.aligner.spectrometer.integration_time = 1000. #long integration time for measurement
            dset2 = g.create_dataset("z_scan2",
                                    data=self.aligner.z_scan(dz))
            for key, val in self.aligner.spectrometer.get_metadata().iteritems():
                dset2.attrs.create(key,val)
            dset2.attrs.create("stage_position",self.csm.stage.position())
            dset2.attrs.create("camera_centre_position",self.csm.camera_centre_position())
            dset2.attrs.create("timestamp",datetime.datetime.now().isoformat())
            dset2.attrs.create("dz",dz)
            #we're going to take a picture - best make sure we've waited a moment for the focus to return
            time.sleep(0.3)
            self.csm.camera.update_latest_frame() #take a frame and ignore (for freshness)
            image = self.csm.camera.color_image()
            img = g.create_dataset("camera_image2",data=image[image.shape[0]/2-50:image.shape[0]/2+50, image.shape[1]/2-50:image.shape[1]/2+50])
            img.attrs.create("stage_position",self.csm.stage.position())
            img.attrs.create("timestamp",datetime.datetime.now().isoformat())
#===================================================================
...
#    spectrometer.show_gui(blocking = False)
    ...
    #len([k for k in scanner.datafile['particleScans/scan1/'].keys() if "z_scan" in k])
