...
# import traits
from traits.api import HasTraits, Enum, Int, Range, Bool, Button,\
    String, on_trait_change, Array
# from traits.api import Float
# import traitsui
from traitsui.api import View, Item, VGroup, Tabbed
# from traitsui.api import HGroup, RangeEditor
import cv2
...
class ParticleScanner(HasTraits):
    ...
    """
    def __init__(self, camera_stage_mapper, spectrometer,
                 spectrometer_aligner, datafile):
    ...
    def threshold_image(self, img):
        ...
        # return cv2.threshold(self.denoise_image(img), self.threshold_level,
        #                     255, cv2.THRESH_BINARY)[1]
        img = cv2.adaptiveThreshold(img, 255, cv2.ADAPTIVE_THRESH_MEAN_C,
                                    cv2.THRESH_BINARY,
                                    (int(self.threshold_block_size)/2)*2+1,
    ...
    # Prevent extra arguments from Traits messing things up
    def find_particles_in_new_image(self):
        self.find_particles()
    ...
    def find_particles(self, img=None):
        ...
        return [np.array(p)+15 for p in  # add 15 onto all the positions
                ndimage.measurements.center_of_mass(img, labels,
                                                    range(1, nlabels+1))]
...
#        self._abort_scan_event.clear()
    ...
    def tile_scans(self, size, background=True, tile_start_function=None,
                   ts_args=[], ts_kwargs={}, *args, **kwargs):
        def worker_function():
            grid_size = np.array(size)
            here = self.csm.camera_centre_position()
            scan_centres = [self.csm.camera_point_to_sample(
                                            np.array([i, j])-grid_size/2)
                            for i in range(grid_size[0])
                            for j in (range(grid_size[1])
                            if i % 2 == 0 else reversed(range(grid_size[1])))
            ...
            latest_group = sorted(
                    [v for k, v in self.datafile['particleScans'].iteritems()
                        if 'scan' in k],
                    key=lambda g: int(re.search(r"(\d+)$",
                                                g.name).groups()[0])
                        )[-1]
            number_of_particles = len([k for k in latest_group.keys()
                                      if 'scan_' in k])
    ...
    def take_zstacks_of_particles(self, dz=np.arange(-2.5, 2.5, 0.2),
                                  datafile_group=None, *args, **kwargs):
    ...
    def take_zstacks_of_particles_tiled(self, shape, **kwargs):
        ...
        We run take_zstacks_of_particles for several fields of view,
    ...
    def save_overview_images(self, datafile_group):
        ...
        """save an unmodified and a thresholded image,
        as a reference for scans"""
        time.sleep(1)
        self.csm.camera.update_latest_frame()
        img1 = datafile_group.create_dataset(self.new_dataset_name(
                datafile_group, "overview_image_%d"),
        ...
        img2 = datafile_group.create_dataset(self.new_dataset_name(
                datafile_group, "overview_image_%d_thresholded"),
                data=self.threshold_image(self.denoise_image(
        ...
        img2.attrs.create("camera_centre_position",
                          self.csm.camera_centre_position())
        for key, val in self.get(
                ['median_filter_width', 'threshold_level']).iteritems():
    ...
    def pf_align_and_do_scan(self,
                             dz=np.arange(-4, 4, 0.4), datafile_group=None):
        ...
        if datafile_group is None:
            datafile_group = self.new_data_group(
                    "particleScans/scan%d", self.datafile)
...
if __name__ == "__main__":
    # from nplab.instrument.spectrometer.seabreeze\
    ...
    scanner = ParticleScanner(mapper, spectrometer, aligner,
                              h5py.File("test_scans.hdf5", 'a'))
    ...
    # len([k for k in scanner.datafile['particleScans/scan1/'].keys()
    #     if "scan" in k])
