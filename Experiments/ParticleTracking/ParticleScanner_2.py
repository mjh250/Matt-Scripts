...
class ParticleScanner(HasTraits):
    ...
    """
    def __init__(self, camera_stage_mapper, spectrometer, spectrometer_aligner, datafile,andor_cam):
        super(ParticleScanner,self).__init__()
        self.csm=camera_stage_mapper
        self.spectrometer = spectrometer
        self.datafile = datafile
        self.aligner = spectrometer_aligner
        self.andor_cam = andor_cam
...
#        self._abort_scan_event.clear()
    ...
    def pf_align_and_take_z_scan(self, dz=np.arange(-4,4,0.4), datafile_group=None):
        ...
        #    image = np.reshape(raman.take_bkg(),(-1, raman.sham.pixel_number))
            image = self.andor_cam.raw_image()
...
if __name__ == "__main__":
 ...
 #   spectrometer = OOSpec(0)
  ...
  #  spectrometer.set_tec_temperature(-21)
    ...
    scanner = ParticleScanner(mapper, spectrometer, aligner,h5py.File("test_zscans.hdf5",'a'),andor_cam)
