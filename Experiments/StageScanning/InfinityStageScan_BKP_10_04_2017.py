...
def GenerateAreaNameArray(scanName, xrng, xstp,
    ...
    #Transpose the output so that picNameArray[x][y] = scanNameXxYy
#    picNameArray = [[j[i] for j in picNameArray]
#                    for i in range(len(picNameArray[0]))]
...
def GenerateVolumeNameArray(scanName, xrng, xstp, yrng, ystp,
    ...
    #Transpose the output so that picNameArray[x][y][z] = scanNameXxYyZz
#    picNameArray = [[j[i] for j in picNameArray]
#                    for i in range(len(picNameArray[0]))]
#    picNameArray = [[[k[j] for k in i]
#                    for j in range(len(i[0]))]
#                    for i in picNameArray]
#    picNameArray = [[j[i] for j in picNameArray]
#                    for i in range(len(picNameArray[0]))]
